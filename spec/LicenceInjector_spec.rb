...
describe LicenceInjector do
	...
	describe "#read_licence_file" do
		...
		it "reads the text from the licence file" do
			licence_injector = LicenceInjector.new :inject,@licence_file, "src_path", ["cpp"], "some old licence path", true
			licence_injector.read_file(@licence_file).should eql @licence_text
		end
	end
	...
	describe "#inject_licence_into_file" do
		...
		context "source file is not empty" do
			...
			it "injects the text of the licence file into the source files" do
				licence_injector = LicenceInjector.new :inject, @licence_file, ".", ["cpp"], "some old licence path", true
				licence_injector.inject_licence_into_file("0.cpp").should eql 1
				get_file_content("0.cpp").should eql (@licence_text + @src_text.join)
			end
		end
		...
		context "source file is empty" do
			...
			it "injects the text of the licence file into the source files" do
				licence_injector = LicenceInjector.new :inject, @licence_file, ".", ["cpp"], "some old licence path", true
				licence_injector.inject_licence_into_file("0.cpp").should eql 1
				get_file_content("0.cpp").should eql (@licence_text)
			end
		end
	end
	...
	describe "#inject_licence_into_file" do
		it "injects the text of the licence file into the source files below the shebang in the source files" do
			...
			licence_injector = LicenceInjector.new :inject,@licence_file, ".", ["cpp"], "some old licence path", true
			licence_injector.inject_licence_into_file("0.cpp").should eql 1
			get_file_content("0.cpp").should eql (@shebang + @licence_text + @src_text.join)
			remove_fake_licence_file @licence_file
			remove_fake_source_files(1, ".cpp")
		end
		...
		it "injects the text of the licence file into the source files below the xml declaration in the source files" do
			...
			licence_injector = LicenceInjector.new :inject,@licence_file, ".", ["xml"], "some old licence path", true
			licence_injector.inject_licence_into_file("0.xml").should eql 1
			get_file_content("0.xml").should eql (@xml_declaration + @xml_licence_text + @xml_text.join)
			remove_fake_licence_file @licence_file
	...
	describe "#inject_licence_into_file" do
		context "many src files" do
			it "injects the text of the licence file into multiple source files" do
				...
				number_of_src_files.times do | x |
					get_file_content(x.to_s + ".cpp").should eql (@licence_text + @src_text.join)
				end
		...
		context "many extensions" do
			it "injects the text of the licence file into multiple source files of differing extensions" do
				...
				licence_injector = LicenceInjector.new :inject,@licence_file, ".", ["cpp", "hpp", "java"], "some old licence path", true
				licence_injector.inject_licence
				...
				number_of_hpp_files.times do | x |
					get_file_content(x.to_s + ".hpp").should eql (@licence_text + @src_text.join)
				end	
				number_of_java_files.times do | x |
					get_file_content(x.to_s + ".java").should eql (@licence_text + @src_text.join)
				end
	...
	describe "#inject_licence_into_file" do
		...
		it "injects the text of the licence file into the source files not in current directory" do
			licence_injector = LicenceInjector.new :inject,@licence_file, src_dir, ["cpp"], "some old licence path", true
			licence_injector.inject_licence
	...
	describe "#replace_old_licence" do
		...
		it "replaces the existing licence with a new licence" do
			licence_injector = LicenceInjector.new :inject, @old_licence_file, ".", ["cpp"], "some old licence path", true
			licence_injector.inject_licence_into_file("0.cpp").should eql 1
			get_file_content("0.cpp").should eql (@old_licence_text + @src_text.join)
			
			new_licence_injector = LicenceInjector.new :replace,@licence_file, ".", ["cpp"], @old_licence_file, true
			new_licence_injector.changed_files_count.should eql 0
			new_licence_injector.replace_old_licence("0.cpp").should eql 1
			get_file_content("0.cpp").should eql (@licence_text + @src_text.join)
		end
	end
	...
	describe "#inject_licence" do
		...
		it "call replace_old_licence when the command is :replace" do
			licence_injector = LicenceInjector.new :replace, @licence_file, ".", ["cpp"], @old_licence_file, true
			
			licence_injector.should_receive(:replace_old_licence).with("./0.cpp").once
			licence_injector.inject_licence
		end
		...
		it "call inject_licence_into_file when the command is :inject" do
			...
			licence_injector.should_receive(:inject_licence_into_file).with("./0.cpp").once
			licence_injector.inject_licence
		end
	end
	...
	describe "#inject_licence_into_file" do
		...
		context "source file is not empty" do
			...
			it "does not change file if --overwrite flag is not passed" do
				licence_injector = LicenceInjector.new :inject, @licence_file, ".", ["cpp"], "some old licence path", true, false
				get_file_content("./0.cpp").should eql @src_text.join
			end
		end
	end
end